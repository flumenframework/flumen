...
import 'package:flumen/managed_auth.dart';
import 'package:flumen_test/flumen_test.dart';
import 'package:test/test.dart';

import 'package:flumen/src/dev/context_helpers.dart';
...
void main() {
  ...
  setUp(() async {
    ...
    var clients = [
      AuthClient("com.flumenframework.app1",
          AuthUtility.generatePasswordHash("kilimanjaro", salt), salt),
      AuthClient("com.flumenframework.app2",
          AuthUtility.generatePasswordHash("fuji", salt), salt),
      AuthClient.withRedirectURI(
          "com.flumenframework.redirect",
          AuthUtility.generatePasswordHash("mckinley", salt),
          salt,
          "http://flumenframework.com/auth/redirect"),
      AuthClient.public("com.flumenframework.public"),
      AuthClient.withRedirectURI(
          "com.flumenframework.redirect2",
          AuthUtility.generatePasswordHash("gibraltar", salt),
          salt,
          "http://flumenframework.com/auth/redirect2")
    ];
  ...
  group("Client behavior", () {
    ...
    test("Get client for ID", () async {
      var c = await auth.getClient("com.flumenframework.app1");
      expect(c is AuthClient, true);
    });

    test("Revoked client can no longer be accessed", () async {
      expect(
          await auth.getClient("com.flumenframework.app1") is AuthClient, true);
      await auth.removeClient("com.flumenframework.app1");
      expect(await auth.getClient("com.flumenframework.app1"), isNull);
    });
  ...
  group("Token behavior via authenticate", () {
    ...
    test(
      ...
      var token = await auth.authenticate(createdUser.username,
          User.defaultPassword, "com.flumenframework.app1", "kilimanjaro");
      expect(token.accessToken, isString);
      expect(token.refreshToken, isString);
      expect(token.clientID, "com.flumenframework.app1");
    ...
    test(
        ...
        () async {
      var token = await auth.authenticate(createdUser.username,
          User.defaultPassword, "com.flumenframework.public", "");
      expect(token.accessToken, isString);
      expect(token.refreshToken, isNull);
      expect(token.clientID, "com.flumenframework.public");
      expect(token.resourceOwnerIdentifier, createdUser.id);
      expect(token.type, "bearer");
      ...
      token = await auth.authenticate(createdUser.username,
          User.defaultPassword, "com.flumenframework.public", null);
      expect(token.accessToken, isString);
      expect(token.refreshToken, isNull);
      expect(token.clientID, "com.flumenframework.public");
      expect(token.resourceOwnerIdentifier, createdUser.id);
    ...
    test("Can create token if client has redirect uri", () async {
      var token = await auth.authenticate(createdUser.username,
          User.defaultPassword, "com.flumenframework.redirect", "mckinley");
      expect(token.accessToken, isString);
      expect(token.refreshToken, isString);
      expect(token.clientID, "com.flumenframework.redirect");
    });

    test("Create token fails if username is incorrect", () async {
      try {
        await auth.authenticate("nonsense", User.defaultPassword,
            "com.flumenframework.app1", "kilimanjaro");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Create token fails if client secret doesn't match", () async {
      try {
        await auth.authenticate(createdUser.username, User.defaultPassword,
            "com.flumenframework.app1", "nonsense");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Create token fails if client secret provided for public client",
      ...
      try {
        await auth.authenticate(createdUser.username, User.defaultPassword,
            "com.flumenframework.public", "nonsense");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
  ...
  group("Refreshing token", () {
    ...
    test(
      ...
      var token = await auth.refresh(
          initialToken.refreshToken, "com.flumenframework.app1", "kilimanjaro");
      ...
      expect(authorization.clientID, "com.flumenframework.app1");
    ...
    test("Can refresh token if client has redirect uri", () async {
      ...
      var refreshToken = await auth.refresh(
          token.refreshToken, "com.flumenframework.redirect", "mckinley");
      expect(refreshToken.accessToken, isString);
      expect(refreshToken.refreshToken, isString);
      expect(refreshToken.clientID, "com.flumenframework.redirect");
    });
    ...
    test("Cannot refresh token that has not been issued", () async {
      try {
        await auth.refresh("nonsense", "com.flumenframework.app1", "kilimanjaro");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Cannot refresh token that is null", () async {
      try {
        await auth.refresh(null, "com.flumenframework.app1", "kilimanjaro");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Cannot refresh wildcard token", () async {
      try {
        await auth.refresh("%", "com.flumenframework.app1", "kilimanjaro");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Cannot refresh token if client id does not match issuing client",
      ...
      try {
        await auth.refresh(
            initialToken.refreshToken, "com.flumenframework.app2", "fuji");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Cannot refresh token if client secret is missing", () async {
      try {
        await auth.refresh(
            initialToken.refreshToken, "com.flumenframework.app1", null);
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Cannot refresh token if client secret is incorrect", () async {
      try {
        await auth.refresh(
            initialToken.refreshToken, "com.flumenframework.app1", "nonsense");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
    ...
    test("Cannot refresh token if owner authentcatable is 'revoked'", () async {
      ...
      try {
        await auth.refresh(
            token.accessToken, "com.flumenframework.redirect", "mckinley");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    });
  ...
  group("Generating auth code", () {
    ...
    test("Can create an auth code that can be exchanged for a token", () async {
      var authCode = await auth.authenticateForCode(createdUser.username,
          User.defaultPassword, "com.flumenframework.redirect");

      expect(authCode.code.length, greaterThan(0));
      expect(authCode.resourceOwnerIdentifier, createdUser.id);
      expect(authCode.clientID, "com.flumenframework.redirect");
      ...
      var token = await auth.exchange(
          authCode.code, "com.flumenframework.redirect", "mckinley");
      expect(token.accessToken, isString);
      expect(token.clientID, "com.flumenframework.redirect");
      expect(token.refreshToken, isString);
      expect(token.resourceOwnerIdentifier, createdUser.id);
    ...
    test("Generate auth code with bad username fails", () async {
      try {
        await auth.authenticateForCode(
            "bob+0@flumen", User.defaultPassword, "com.flumenframework.redirect");
        expect(true, false);
      } on AuthServerException catch (e) {
        expect(e.client.id, "com.flumenframework.redirect");
    ...
    test("Generate auth code with bad password fails", () async {
      try {
        await auth.authenticateForCode(
            createdUser.username, "foobaraxegri%", "com.flumenframework.redirect");
        expect(true, false);
      } on AuthServerException catch (e) {
        expect(e.client.id, "com.flumenframework.redirect");
        expect(e.reason, AuthRequestError.accessDenied);
    ...
    test("Generate auth code with wildcard password fails", () async {
      try {
        await auth.authenticateForCode(
            createdUser.username, "%", "com.flumenframework.redirect");
        expect(true, false);
      } on AuthServerException catch (e) {
        expect(e.client.id, "com.flumenframework.redirect");
        expect(e.reason, AuthRequestError.accessDenied);
    ...
    test("Generate auth code with no redirect uri fails", () async {
      try {
        await auth.authenticateForCode(createdUser.username,
            User.defaultPassword, "com.flumenframework.app1");
        expect(true, false);
      } on AuthServerException catch (e) {
        expect(e.client.id, "com.flumenframework.app1");
  ...
  group("Exchanging auth code", () {
    ...
    test("Can create an auth code that can be exchanged for a token", () async {
      var token = await auth.exchange(
          code.code, "com.flumenframework.redirect", "mckinley");
      expect(token.accessToken, isString);
      expect(token.refreshToken, isString);
      expect(token.clientID, "com.flumenframework.redirect");
      expect(token.resourceOwnerIdentifier, createdUser.id);
    ...
    test("Null code fails", () async {
      try {
        await auth.exchange(null, "com.flumenframework.redirect", "mckinley");
    ...
    test("Wildcard code fails", () async {
      try {
        await auth.exchange("%", "com.flumenframework.redirect", "mckinley");
    ...
    test("Code that doesn't exist fails", () async {
      try {
        await auth.exchange("foobar", "com.flumenframework.redirect", "mckinley");
    ...
    test("Expired code fails and it gets deleted", () async {
      ...
      try {
        await auth.exchange(code.code, "com.flumenframework.redirect", "mckinley");
    ...
    test(
      ...
      var refreshedToken = await auth.refresh(
          issuedToken.refreshToken, "com.flumenframework.redirect", "mckinley");
    ...
    test("Different client ID than the one that generated code fials",
      ...
      try {
        await auth.exchange(
            code.code, "com.flumenframework.redirect2", "gibraltar");
    ...
    test("No client secret fails", () async {
      try {
        await auth.exchange(code.code, "com.flumenframework.redirect", null);
    ...
    test("Wrong client secret fails", () async {
      try {
        await auth.exchange(code.code, "com.flumenframework.redirect", "nonsense");
  ...
  group("Cleanup/friendly fire scenarios from client", () {
    ...
    test("Revoking a client revokes all of its tokens and auth codes",
      ...
      var exchangedToken = await auth.exchange(
          exchangedCode.code, "com.flumenframework.redirect", "mckinley");
      ...
      await auth.removeClient("com.flumenframework.redirect");
      try {
        await auth.exchange(
            unusedCode.code, "com.flumenframework.redirect", "mckinley");
        expect(true, false);
        // ignore: empty_catches
      } on AuthServerException {}
    ...
    test(
      ...
      await auth.authenticateForCode(createdUsers.first.username,
          User.defaultPassword, "com.flumenframework.redirect");
      await auth.exchange(
          exchangedCodeRevoke.code, "com.flumenframework.redirect", "mckinley");
      await auth.authenticate(createdUsers.first.username, User.defaultPassword,
          "com.flumenframework.redirect", "mckinley");
      ...
      var exchangedTokenKeep = await auth.exchange(
          exchangedCodeKeep.code, "com.flumenframework.redirect2", "gibraltar");
      var issuedTokenKeep = await auth.authenticate(createdUsers.first.username,
          User.defaultPassword, "com.flumenframework.redirect2", "gibraltar");

      await auth.removeClient("com.flumenframework.redirect");

      var exchangedLater = await auth.exchange(
          unusedCodeKeep.code, "com.flumenframework.redirect2", "gibraltar");
    ...
    test("Clients retain their token ownership", () async {
      ...
      expect(p1.clientID, "com.flumenframework.app1");
      ...
      expect(p2.clientID, "com.flumenframework.redirect");
  ...
  group("Code friendly fire/cleanup", () {
    ...
    test(
      ...
      var manualCode = ManagedAuthToken()
        ...
        ..client = (ManagedAuthClient()..id = "com.flumenframework.redirect")
      ...
      var lastCode = await auth.authenticateForCode(createdUsers.first.username,
          User.defaultPassword, "com.flumenframework.redirect2");
  ...
  group("Token friendly fire/cleanup", () {
    ...
    test(
      ...
      var manualToken = ManagedAuthToken()
        ...
        ..issueDate = DateTime.now().toUtc()
        ..expirationDate = DateTime.now().add(const Duration(seconds: 60)).toUtc()
        ..client = (ManagedAuthClient()..id = "com.flumenframework.app1")
        ..resourceOwner = (User()..id = createdUsers.first.id);
      ...
      var lastToken = await auth.authenticate(createdUsers.first.username,
          User.defaultPassword, "com.flumenframework.app2", "fuji");
      tokensInDB =
          (await tokenQuery.fetch()).map((ac) => ac.accessToken).toList();
    ...
    test("Ensure users aren't authenticated by other users", () async {
      ...
      expect(permission.clientID, "com.flumenframework.app1");
...
Future<List<User>> createUsers(ManagedContext ctx, int count) async {
  ...
  for (int i = 0; i < count; i++) {
    ...
    var u = User()
      ..username = "bob+$i@flumenframework.com"
      ..salt = salt
      ..hashedPassword =
