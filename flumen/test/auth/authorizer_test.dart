...
void main() {
  ...
  group("Bearer Token", () {
    ...
    test("Valid bearer token returns authorization object", () async {
      ...
      expect(json.decode(res.body), {
        "clientID": "com.flumenframework.app1",
  ...
  group("Basic Credentials", () {
    ...
    test("Invalid client secret returns 401", () async {
      ...
      var res = await http.get("http://localhost:8000", headers: {
        HttpHeaders.authorizationHeader:
            "Basic ${const Base64Encoder().convert("com.flumenframework.app1:foobar".codeUnits)}"
      });
      expect(res.statusCode, 401);
      expect(res.body, "");
    });
    ...
    test("Valid client ID returns 200 with authorization", () async {
      ...
      var res = await http.get("http://localhost:8000", headers: {
        HttpHeaders.authorizationHeader:
            "Basic ${const Base64Encoder().convert("com.flumenframework.app1:kilimanjaro".codeUnits)}"
      });
      expect(res.statusCode, 200);
      expect(json.decode(res.body), {
        "clientID": "com.flumenframework.app1",
        "resourceOwnerIdentifier": null,
        "credentials": "com.flumenframework.app1:kilimanjaro"
      });
    });
    ...
    test("Public client can only be authorized with no password", () async {
      ...
      var res = await http.get("http://localhost:8000", headers: {
        HttpHeaders.authorizationHeader:
            "Basic ${const Base64Encoder().convert("com.flumenframework.public:".codeUnits)}"
      });
      expect(res.statusCode, 200);
      expect(json.decode(res.body), {
        "clientID": "com.flumenframework.public",
        "resourceOwnerIdentifier": null,
        "credentials": "com.flumenframework.public:"
      });

      res = await http.get("http://localhost:8000", headers: {
        HttpHeaders.authorizationHeader:
            "Basic ${const Base64Encoder().convert("com.flumenframework.public:password".codeUnits)}"
      });
      expect(res.statusCode, 401);
    });
    ...
    test("Confidential client can never be authorized with no password",
      ...
      var res = await http.get("http://localhost:8000", headers: {
        HttpHeaders.authorizationHeader:
            "Basic ${const Base64Encoder().convert("com.flumenframework.app1:".codeUnits)}"
      });
      expect(res.statusCode, 401);
