...
void main() {
  ...
  group("Success Cases: password", () {
    test("Confidental Client has all parameters including refresh_token",
        () async {
      var res = await grant("com.flumenframework.app1", "kilimanjaro", user1);

      expect(res, hasAuthResponse(200, bearerTokenMatcher));
    });

    test("Public Client has all parameters except refresh_token", () async {
      var res = await grant("com.flumenframework.public", "", user1);
    ...
    test(
        ...
        () async {
      var res = await grant("com.flumenframework.redirect", "mckinley", user1);
      expect(res, hasAuthResponse(200, bearerTokenMatcher));
    });
    ...
    test("Can be scoped", () async {
      ...
      var res = await grant("com.flumenframework.scoped", "kilimanjaro", m);
      expect(res, hasAuthResponse(200, bearerTokenMatcherWithScope("user")));

      m["scope"] = "user other_scope";
      res = await grant("com.flumenframework.scoped", "kilimanjaro", m);
      expect(
          res,
          hasAuthResponse(
  ...
  group("Success Cases: refresh_token", () {
    test(
        ...
        () async {
      var resToken = await grant("com.flumenframework.app1", "kilimanjaro", user1);

      var resRefresh = await refresh("com.flumenframework.app1", "kilimanjaro",
    ...
    test("If token is scoped and scope is omitted, get same token back",
      ...
      var resToken = await grant("com.flumenframework.scoped", "kilimanjaro", m);

      var resRefresh = await refresh("com.flumenframework.scoped", "kilimanjaro",
          refreshTokenMapFromTokenResponse(resToken));
      expect(
          resRefresh,
          hasResponse(200, body: {
    ...
    test("If token is scoped and scope is part of request, get rescoped token",
      ...
      var resRefresh =
          await refresh("com.flumenframework.scoped", "kilimanjaro", refreshMap);
      expect(
          resRefresh,
          hasResponse(200, body: {
            "access_token": isString,
  ...
  group("Success Cases: authorization_code", () {
    test("Exchange valid code gets new access token with refresh token",
      ...
      var code = await authenticationServer.authenticateForCode(
          user1["username"], user1["password"], "com.flumenframework.redirect");
      var res =
          await exchange("com.flumenframework.redirect", "mckinley", code.code);
      expect(res, hasAuthResponse(200, bearerTokenMatcher));
    });

    test("If code is scoped, token has same scope", () async {
      var code = await authenticationServer.authenticateForCode(
          user1["username"], user1["password"], "com.flumenframework.scoped",
          requestedScopes: [AuthScope("user")]);

      var res =
          await exchange("com.flumenframework.scoped", "kilimanjaro", code.code);
      expect(res, hasAuthResponse(200, bearerTokenMatcherWithScope("user")));
    });
  });
  ...
  group("username Failure Cases", () {
    test("Username does not exist yields 400", () async {
      var resToken = await grant("com.flumenframework.app1", "kilimanjaro",
    ...
    test("Username is missing returns 400", () async {
      var resToken = await grant(
          "com.flumenframework.app1", "kilimanjaro", {"password": "doesntmatter"});
    ...
    test("Username is repeated returns 400", () async {
      ...
      final client = Agent.onPort(8888)
        ..headers["authorization"] =
            "Basic ${base64.encode("com.flumenframework.app1:kilimanjaro".codeUnits)}";
  ...
  group("code Failure Cases", () {
    test("code is invalid (not issued)", () async {
      ...
      var res = await exchange(
          "com.flumenframework.redirect", "mckinley", "a${code.code}");
      expect(res, hasResponse(400, body: {"error": "invalid_grant"}));
    });

    test("code is missing", () async {
      var res = await exchange("com.flumenframework.redirect", "mckinley", null);
      expect(res, hasResponse(400, body: {"error": "invalid_request"}));
    });

    test("code is empty", () async {
      var res = await exchange("com.flumenframework.redirect", "mckinley", "");
      expect(res, hasResponse(400, body: {"error": "invalid_grant"}));
    });
    ...
    test("code is duplicated", () async {
      ...
      final client = Agent.onPort(8888)
        ..setBasicAuthorization("com.flumenframework.redirect", "mckinley");
    ...
    test("code is from a different client", () async {
      var code = await authenticationServer.authenticateForCode(
          user1["username"], user1["password"], "com.flumenframework.redirect");
      var res =
          await exchange("com.flumenframework.redirect2", "gibraltar", code.code);
      expect(res, hasResponse(400, body: {"error": "invalid_grant"}));
    });
  });
  ...
  group("grant_type Failure Cases", () {
    ...
    setUp(() {
      client = Agent.onPort(8888)
        ..setBasicAuthorization("com.flumenframework.app1", "kilimanjaro");
    });
    ...
    test("Duplicate grant_type", () async {
      client.setBasicAuthorization("com.flumenframework.redirect", "mckinley");
  ...
  group("refresh_token Failure Cases", () {
    ...
    test("refresh_token is omitted", () async {
      ...
      var resRefresh = await refresh("com.flumenframework.app1", "kilimanjaro", m);
    ...
    test("refresh_token appears more than once", () async {
      final Map<String, dynamic> grantMap =
          (await grant("com.flumenframework.app1", "kilimanjaro", user1))
              .body
              .as();
      final refreshToken =
    ...
    test("Client id/secret pair is different than original", () async {
      ...
      var resRefresh = await refresh("com.flumenframework.app2", "fuji",
          refreshTokenMapFromTokenResponse(resToken));
      expect(resRefresh, hasResponse(400, body: {"error": "invalid_grant"}));
    });
  });
  ...
  group("Authorization Header Failure Cases (password grant_type)", () {
    ...
    test("Confidential client has wrong secret", () async {
      var resp = await grant("com.flumenframework.app1", "notright", user1);
    ...
    test(
        ...
        () async {
      var resp = await grant("com.flumenframework.app1", "", user1);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("Public client has wrong secret (any secret)", () async {
      var resp = await grant("com.flumenframework.public", "foo", user1);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("Confidential Client ID doesn't exist", () async {
      var resp = await grant("com.flumenframework.app123", "foo", user1);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("Public Client ID doesn't exist", () async {
      var resp = await grant("com.flumenframework.app123", "", user1);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });
  });
  ...
  group("Authorization Header Failure Cases (refresh_token grant_type)", () {
    ...
    test("Confidential client has wrong secret", () async {
      var resp = await refresh("com.flumenframework.app1", "notright",
          {"refresh_token": refreshTokenString});
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });
    ...
    test("Confidential client can't be used as a public client", () async {
      var resp = await refresh(
          "com.flumenframework.app1", "", {"refresh_token": refreshTokenString});
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("Confidential Client ID doesn't exist", () async {
      var resp = await refresh("com.flumenframework.app123", "foo",
          {"refresh_token": refreshTokenString});
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("client id is different than one that generated token", () async {
      var resp = await refresh("com.flumenframework.app2", "fuji",
          {"refresh_token": refreshTokenString});
  ...
  group("Authorization Header Failure Cases (authorization_code grant_type)",
    ...
    setUp(() async {
      ...
      code = (await authenticationServer.authenticateForCode(user1["username"],
              user1["password"], "com.flumenframework.redirect"))
          .code;
    });
    ...
    test("Confidential client has wrong secret", () async {
      var resp = await exchange("com.flumenframework.redirect", "notright", code);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("Confidential client can't be used as a public client", () async {
      var resp = await exchange("com.flumenframework.redirect", "", code);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("Confidential Client ID doesn't exist", () async {
      var resp = await exchange("com.flumenframework.app123", "foo", code);
      expect(resp, hasResponse(400, body: {"error": "invalid_client"}));
    });

    test("client id is different than one that generated token", () async {
      var resp =
          await exchange("com.flumenframework.redirect2", "gibraltar", code);
      expect(resp, hasResponse(400, body: {"error": "invalid_grant"}));
    });
  });
  ...
  group("Scope failure cases", () {
    test("Try to add scope to code exchange is invalid_request", () async {
      ...
      final client = Agent.onPort(8888)
        ..setBasicAuthorization("com.flumenframework.scoped", "kilimanjaro");
      var req = client.request("/auth/token")
        ..contentType = ContentType("application", "x-www-form-urlencoded")
        ..body = m;
