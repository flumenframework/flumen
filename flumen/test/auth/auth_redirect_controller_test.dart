...
import 'package:flumen/src/dev/helpers.dart';
...
void main() {
  ...
  group("GET success case", () {
    test(
      ...
      final req = client.request("/auth/code")
        ..query = {
          "client_id": "com.flumenframework.redirect",
          "response_type": "code"
        };
    ...
    test(
      ...
      final req = client.request("/auth/redirect")
        ..query = {
          "client_id": "com.flumenframework.public.redirect",
          "response_type": "token"
        };
  ...
  group("Code Success cases", () {
    test("With required values", () async {
      final resp = await codeResponse({
        "client_id": "com.flumenframework.redirect",
        "state": "Wisconsin@&",
        "username": "bob+0@flumenframework.com",
        "password": InMemoryAuthStorage.defaultPassword
      });

      expectRedirect(resp, Uri.http("flumenframework.com", "/auth/redirect"),
          state: "Wisconsin@&");
    });

    test("With scope", () async {
      final resp = await codeResponse({
        "client_id": "com.flumenframework.scoped",
        "state": "Wisconsin@&",
        "username": "bob+0@flumenframework.com",
        "password": InMemoryAuthStorage.defaultPassword,
        "scope": "user"
      ...
      expectRedirect(resp, Uri.http("flumenframework.com", "/auth/scoped"),
          state: "Wisconsin@&");
      ...
      final token = await application.channel.authServer
          .exchange(codeParam, "com.flumenframework.scoped", "kilimanjaro");
  ...
  group("Token Success cases", () {
    test("With required values", () async {
      ...
      expectTokenRedirect(resp, Uri.http("flumenframework.com", "/auth/public-redirect"),
          state: "Wisconsin@&");
    });

    test("With scope", () async {
      final resp = await tokenResponse({
        "client_id": "com.flumenframework.public.scoped",
        "state": "Wisconsin@&",
        "username": "bob+0@flumenframework.com",
        "password": InMemoryAuthStorage.defaultPassword,
        "scope": "user"
      ...
      expectTokenRedirect(resp, Uri.http("flumenframework.com", "/auth/public-scoped"),
          state: "Wisconsin@&");
  ...
  group("Code username Failure Cases", () {
    test("Username does not exist yields 302 with error", () async {
      ...
      expectErrorRedirect(
          resp, Uri.http("flumenframework.com", "/auth/redirect"), "access_denied",
          state: "a");
    });
    ...
    test("Username is missing returns 302 with error", () async {
      ...
      expectErrorRedirect(resp, Uri.http("flumenframework.com", "/auth/redirect"),
          "invalid_request",
          state: "a");
    });
    ...
    test("Username is repeated returns 400", () async {
      ...
      var req = client.request("/auth/code")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&username=$encodedWrongUsername&password=$encodedPassword&response_type=code&client_id=com.flumenframework.redirect&state=a")
      ...
      req = client.request("/auth/code")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedWrongUsername&username=$encodedUsername&password=$encodedPassword&response_type=code&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasStatus(400));
    });
  ...
  group("Token username Failure Cases", () {
    test("Username does not exist yields 302 with error", () async {
      ...
      expectTokenErrorRedirect(
          resp, Uri.http("flumenframework.com", "/auth/public-redirect"), "access_denied",
          state: "a");
    });
    ...
    test("Username is missing returns 302 with error", () async {
      ...
      expectTokenErrorRedirect(resp, Uri.http("flumenframework.com", "/auth/public-redirect"),
          "invalid_request",
          state: "a");
    });
    ...
    test("Username is repeated returns 400", () async {
      ...
      var req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&username=$encodedWrongUsername&password=$encodedPassword&response_type=token&client_id=com.flumenframework.public.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      var resp = await req.post();
      expect(resp, hasStatus(400));
      ...
      req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedWrongUsername&username=$encodedUsername&password=$encodedPassword&response_type=token&client_id=com.flumenframework.public.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasStatus(400));
    });
  ...
  group("Code password Failure Cases", () {
    ...
    test("password is repeated returns 302 with error", () async {
      ...
      var req = client.request("/auth/code")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedWrongPassword&password=$encodedPassword&response_type=code&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      var resp = await req.post();
      expect(resp, hasStatus(400));
      ...
      req = client.request("/auth/code")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&password=$encodedWrongPassword&response_type=code&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasStatus(400));
    });
  ...
  group("Token password Failure Cases", () {
    ...
    test("password is repeated returns 302 with error", () async {
      ...
      var req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedWrongPassword&password=$encodedPassword&response_type=code&client_id=com.flumenframework.public.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      var resp = await req.post();
      expect(resp, hasStatus(400));
      ...
      req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&password=$encodedWrongPassword&response_type=code&client_id=com.flumenframework.public.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasStatus(400));
    });
  ...
  group("response_type failures", () {
    test("response_type is invalid returns 400 with error", () async {
      ...
      final req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&response_type=notcode&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
    ...
    test("response_type is duplicated returns 302 with error", () async {
      ...
      var req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&response_type=notcode&response_type=code&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      var resp = await req.post();
      expect(resp, hasStatus(400));
      ...
      req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&response_type=code&response_type=notcode&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasStatus(400));
    });
  ...
  group("client_id failures", () {
    ...
    test("client_id that does not have redirectURI returns 400", () async {
      final resp = await tokenResponse({
        "client_id": "com.flumenframework.app1",
        "username": user1["username"],
        "password": user1["password"],
        "state": "a"
      });
    ...
    test("client_id is duplicated returns 400", () async {
      ...
      var req = client.request("/auth/redirect")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&response_type=code&client_id=com.flumenframework.redirect&client_id=foobar&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      var resp = await req.post();
      expect(resp, hasStatus(400));
      ...
      req = client.request("/auth/code")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&response_type=code&client_id=foobar&client_id=com.flumenframework.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      resp = await req.post();
      expect(resp, hasStatus(400));
    });
  ...
  group("Code Invalid requests and state", () {
    test("public client with response type code redirects with error", () async {
      ...
      expectErrorRedirect(resp, Uri.http("flumenframework.com", "/auth/public-redirect"), "unauthorized_client", state: "a");
    });
    ...
    test("Omit state is error", () async {
      ...
      expectErrorRedirect(resp, Uri.http("flumenframework.com", "/auth/redirect"), "invalid_request");
    });
  ...
  group("Token Invalid requests and state", () {
    test("Does not allow response_type of token if allowsImplicit is false", () async {
      ...
      final req = client.request("/auth/code")
        ..encodeBody = false
        ..body = utf8.encode(
            "username=$encodedUsername&password=$encodedPassword&response_type=token&client_id=com.flumenframework.public.redirect&state=a")
        ..contentType = ContentType("application", "x-www-form-urlencoded");
      final resp = await req.post();
      expect(resp, hasStatus(400));
      expect(resp, hasHeaders({"content-type": "text/html; charset=utf-8"}));
    ...
    test("Omit state is error", () async {
      ...
      expectTokenErrorRedirect(resp, Uri.http("flumenframework.com", "/auth/public-redirect"), "invalid_request");
    });
  ...
  group("Scope failure cases", () {
    test("Malformed scope", () async {
      ...
      expectErrorRedirect(
          resp, Uri.http("flumenframework.com", "/auth/scoped"), "invalid_scope",
          state: "Wisconsin@&");
    });
...
Map<String, String> get user2 => const {
      "username": "bob+1@flumenframework.com",
      "password": InMemoryAuthStorage.defaultPassword
    };

Map<String, String> get user3 => const {
      "username": "bob+2@flumenframework.com",
      "password": InMemoryAuthStorage.defaultPassword
    };
