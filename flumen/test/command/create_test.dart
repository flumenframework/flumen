...
void main() {
  ...
  group("Templates", () {
    ...
    test("Template gets generated from local path, project points to it",
      ...
      var flumenLocationString = File.fromUri(cli
              .agent.workingDirectory.uri
              .resolve("test_project/")
              .resolve(".packages"))
          .readAsStringSync()
          .split("\n")
          .firstWhere((p) => p.startsWith("flumen:"))
          .split("flumen:")
          .last;

      var path = path_lib.normalize(path_lib.fromUri(flumenLocationString));
    ...
    final templates = Directory("templates")
        ...
        .toList();
    final flumenPubspec = loadYaml(File("pubspec.yaml").readAsStringSync());
    final flumenVersion = Version.parse("${flumenPubspec["version"]}");

    for (var template in templates) {
      test("Templates can use 'this' version of Flumen in their dependencies",
          () {
        ...
        final projectVersionConstraint = VersionConstraint.parse(
            contents["dependencies"]["flumen"] as String);
        expect(projectVersionConstraint.allows(flumenVersion), true);
      });
