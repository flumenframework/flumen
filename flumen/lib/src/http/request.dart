...
class Request implements RequestOrResponse {
  ...
  ///
  /// You should typically not manipulate this property's [HttpRequest.response]. By default, Flumen controls
  /// the response through its [Controller]s.
  ///
  /// If you wish to respond to a request manually - and prohibit Flumen from responding to the request - you must
  ...
  ///
  /// Once this method has executed, the [Request] is no longer valid. All headers from [flumenResponse] are
  /// added to the HTTP response. If [flumenResponse] has a [Response.body], this request will attempt to encode the body data according to the
  /// Content-Type in the [flumenResponse]'s [Response.headers].
  ///
  Future respond(Response flumenResponse) {
    respondDate = DateTime.now().toUtc();

    final modifiers = _responseModifiers;
    _responseModifiers = null;
    modifiers?.forEach((modifier) {
      modifier(flumenResponse);
    });

    _Reference<String> compressionType = _Reference(null);
    var body = flumenResponse.body;
    if (body is! Stream) {
      ...
      body = _responseBodyBytes(flumenResponse, compressionType);
    }

    response.statusCode = flumenResponse.statusCode;
    flumenResponse.headers?.forEach((k, v) {
      response.headers.add(k, v);
    });

    if (flumenResponse.cachePolicy != null) {
      response.headers.add(HttpHeaders.cacheControlHeader,
          flumenResponse.cachePolicy.headerValue);
    }
    ...
    response.headers.add(
        HttpHeaders.contentTypeHeader, flumenResponse.contentType.toString());
    ...
    if (body is List<int>) {
      ...
      final bodyStream = _responseBodyStream(flumenResponse, compressionType);
      if (compressionType.value != null) {
        response.headers
            .add(HttpHeaders.contentEncodingHeader, compressionType.value);
      }
      response.headers.add(HttpHeaders.transferEncodingHeader, "chunked");
      response.bufferOutput = flumenResponse.bufferOutput;
