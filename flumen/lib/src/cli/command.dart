...
abstract class CLICommand {
  ...
  Future<int> process(args.ArgResults results,
    ...
    try {
      ...
      if (showVersion) {
        outputSink.writeln("Flumen CLI version: $toolVersion");
        return 0;
      }

      if (!isMachineOutput) {
        displayInfo("Flumen CLI Version: $toolVersion");
      }
  ...
  Future determineToolVersion() async {
    try {
      var toolLibraryFilePath = (await Isolate.resolvePackageUri(
              currentMirrorSystem().findLibrary(#flumen).uri))
          .toFilePath(windows: Platform.isWindows);
      var flumenDirectory = Directory(FileSystemEntity.parentOf(
          FileSystemEntity.parentOf(toolLibraryFilePath)));
      var toolPubspecFile =
          File.fromUri(flumenDirectory.absolute.uri.resolve("pubspec.yaml"));
