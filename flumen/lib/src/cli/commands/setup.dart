...
class CLISetup extends CLICommand with CLIProject {
  ...
  Future<int> handle() async {
    if (shouldSetupHeroku) {
      ...
      displayProgress("Please see TBD for instructions.");
      return 0;
  ...
  Future<int> setupTestEnvironment() async {
    if (!hasPSQLCLI) {
      displayError(
          ...
          "you must do so to run tests in an Flumen application. For macOS users, "
    ...
    displayInfo(
        "Congratulations! Flumen applications can now be tested locally.");
