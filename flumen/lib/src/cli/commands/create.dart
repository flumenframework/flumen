...
/// Used internally.
class CLITemplateCreator extends CLICommand with CLIFlumenGlobal {
  CLITemplateCreator() {
  ...
  Future<int> handle() async {
    if (projectName == null) {
      printHelp(parentCommandName: "flumen");
      return 1;
    }
    ...
    displayProgress("See more templates with 'flumen create list-templates'");
    copyProjectFiles(destDirectory, templateSourceDirectory, projectName);

    createProjectSpecificFiles(destDirectory.path);
    if (flumenPackageRef.sourceType == "path") {
      final flumenLocation = flumenPackageRef.resolve().location;

      addDependencyOverridesToPackage(destDirectory.path, {
        "flumen": flumenLocation.uri,
        "flumen_test": flumenLocation.parent.uri.resolve("test/")
      });
    }
  ...
  String get detailedDescription {
    return "This command will use a template from the flumen package determined by either "
  ...
  String get description {
    return "Creates Flumen applications from templates.";
  }
}

class CLITemplateList extends CLICommand with CLIFlumenGlobal {
  @override
  Future<int> handle() async {
  ...
  @override
  String get description {
    return "List Flumen application templates.";
  }
...
class CLIFlumenGlobal {
  PubCache pub = PubCache();

  PackageRef get flumenPackageRef {
    return pub
        .getGlobalApplications()
        .firstWhere((app) => app.name == "flumen")
        .getDefiningPackageRef();
  }

  Uri get templateDirectory {
    return flumenPackageRef.resolve().location.uri.resolve("templates/");
  }
