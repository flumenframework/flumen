...
abstract class CLIProject implements CLICommand {
  ...
  Version get projectVersion {
    if (_projectVersion == null) {
      ...
      final projectVersion =
          lockFileContents["packages"]["flumen"]["version"] as String;
  ...
  void preProcess() {
    try {
      if (!isMachineOutput) {
        displayInfo("Flumen project version: $projectVersion");
      }

      if (projectVersion?.major != toolVersion.major) {
        throw CLIException(
            "CLI version is incompatible with project flumen version.",
            instructions: [
              "Install flumen@${projectVersion?.toString()} or upgrade your project to flumen{toolVersion.toString()}."
            ]);
      }
    } on CLIException {
      rethrow;
